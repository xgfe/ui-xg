...
describe('ui.xg.progressbar directive', function () {
    var $rootScope, $compile, element;

    beforeEach(module('ui.xg.progressbar'));
    beforeEach(module('progressbar/templates/progressbar.html', 'progressbar/templates/progress.html', 'progressbar/templates/bar.html'));
    beforeEach(inject(function (_$compile_, _$rootScope_) {
        $compile = _$compile_;
        $rootScope = _$rootScope_;
        $rootScope.value = 22;
        element = $compile('<uix-progressbar animate="false" value="value" title="foo">{{value}} %</uix-progressbar>')($rootScope);
        $rootScope.$digest();
    }));
    ...
    it('has the appropriate aria markup', function () {
        ...
        expect(bar.attr('title')).toBe('foo');
    });

    it('has the default aria-labelledby value of `progressbar`', function () {
        element = $compile('<uix-progressbar animate="false" value="value">{{value}} %</uix-progressbar>')($rootScope);
        $rootScope.$digest();
        var bar = getBar(0);
        expect(bar.attr('title')).toBe('progressbar');
    });
    ...
    it('it should be possible to add additional classes', function () {
        element = $compile('<uix-progress class="progress-striped active" max="200"><uix-bar class="pizza"></uix-bar></uix-progress>')($rootScope);
        $rootScope.$digest();
    ...
    describe('"max" attribute', function () {
        beforeEach(inject(function () {
            $rootScope.max = 200;
            element = $compile('<uix-progressbar max="max" animate="false" value="value">{{value}}/{{max}}</uix-progressbar>')($rootScope);
            $rootScope.$digest();
        }));
    ...
    describe('"max" attribute using object', function () {
        beforeEach(inject(function () {
            element = $compile('<uix-progressbar max="settings.max" animate="false" value="settings.value">{{settings.value}}/{{settings.max}}</uix-progressbar>')($rootScope);
            $rootScope.$digest();
        }));
    ...
    describe('"type" attribute', function () {
        beforeEach(inject(function () {
            ...
            element = $compile('<uix-progressbar value="value" type="{{type}}"></uix-progressbar>')($rootScope);
            $rootScope.$digest();
        }));
    ...
    describe('stacked', function () {
        beforeEach(inject(function () {
            ...
            element = $compile('<uix-progress animate="false"><uix-bar ng-repeat="o in objects" value="o.value" ' +
                               'type="{{o.type}}" title="{{o.title}}">{{o.value}}</uix-bar></uix-progress>')($rootScope);
            $rootScope.$digest();
        }));
        ...
        it('uses correct class', function () {
            expect(getBar(0)).toHaveClass('progress-bar-success');
            expect(getBar(0)).not.toHaveClass('progress-bar-warning');

            expect(getBar(1)).toHaveClass('progress-bar-warning');
            expect(getBar(1)).not.toHaveClass('progress-bar-danger');

            expect(getBar(2)).not.toHaveClass('progress-bar-success');
            expect(getBar(2)).not.toHaveClass('progress-bar-warning');
        });

        it('should change classes if type changed', function () {
            $rootScope.objects = [
                { value: 20, title: 'foo', type: 'warning' },
                { value: 50, title: 'bar'},
                { value: 30, title: 'baz', type: 'success'}
            ];
            $rootScope.$digest();

            expect(getBar(0)).toHaveClass('progress-bar-warning');
            expect(getBar(0)).not.toHaveClass('progress-bar-success');

            expect(getBar(1)).not.toHaveClass('progress-bar-warning');
            expect(getBar(1)).not.toHaveClass('progress-bar-danger');

            expect(getBar(2)).toHaveClass('progress-bar-success');
            expect(getBar(2)).not.toHaveClass('progress-bar-warning');
        });
        ...
        it('should default to progressbar', function () {
            $rootScope.objects = [
                ...
                { value: 50, type: 'info'},
                { value: 30, type: 'success'}
            ];
            element = $compile('<uix-progress animate="false"><uix-bar ng-repeat="o in objects" value="o.value" type="{{o.type}}">' +
                               '</uix-bar></uix-progress>')($rootScope);
            $rootScope.$digest();
            for (var i = 0; i < 3; i++) {
                expect(getBar(i).attr('title')).toEqual('progressbar');
            }
        });
        ...
        describe('max attribute', function () {
            beforeEach(inject(function () {
                $rootScope.max = 200;
                element = $compile('<uix-progress max="max" animate="false"><uix-bar ng-repeat="o in objects" value="o.value" ' +
                                   'type="{{o.type}}"></uix-bar></uix-progress>')($rootScope);
                $rootScope.$digest();
            }));

            it('correct max value', function () {
                expect(getBar(0).attr('aria-valuemax')).toBe('200');
            });

            it('adjust the correct width when it changes', function () {
            ...
            it('adjust the correct width when value changes', function () {
                expect(getBar(1).css('width')).toBe('25%');

                $rootScope.objects[1].value = 60;
                $rootScope.$digest();
                expect(getBar(1).css('width')).toBe('30%');
            ...
            it('correct bar text', function () {
                expect(getBar(0).attr('aria-valuetext')).toBe('5%');
            });

            it('adjust max value when it changes', function () {
                expect(getBar(0).attr('aria-valuemax')).toBe('200');
            ...
            it('should not have a total width over 100%', function () {
                $rootScope.objects = [
                    { value: 136, type: 'warning' },
                    { value: 80, type: 'info'},
                    { value: 122, type: 'success'}
                ];
                $rootScope.max = 338;
                $rootScope.$digest();
                var totalWidth = 0;
            ...
            it('should have a total width 63.91 when remove a bar', function () {
                $rootScope.objects = [
                    { value: 136, type: 'warning' },
                    { value: 80, type: 'info'},
                    { value: 122, type: 'success'}
                ...
                totalWidth = 0;
                for (var j = 0; j < 2; j++) {
                    totalWidth += parseFloat(getBar(j).css('width'));
                }
                expect(totalWidth.toFixed(2)).toBe('63.91');
            });
        });
    });
});
